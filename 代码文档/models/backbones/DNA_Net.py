...
class DNANet(BaseModule):
    def __init__(self,
                 input_channels=3,
                 num_blocks=[2, 2, 2, 2],
                 nb_filter=[16, 32, 64, 128, 256],
                 max_downsample=16,  # 新增控制参数：8 或 16
                 deep_supervision=False,
                 init_cfg=None):
        super(DNANet, self).__init__(init_cfg)
        ...
        block = Res_CBAM_block  # 默认使用 Res_CBAM
        ...
        # Final Fusion Inputs for Stage 0
        ...
        if self.max_downsample == 16:
            # === 原始完整结构 ===
            # 需要 Stage 4 (Stride 16)
            self.conv4_0 = self._make_layer(block, nb_filter[3], nb_filter[4], num_blocks[3])

            # conv3_1 需要来自 conv4_0 的上采样 + 来自 conv2_1 的下采样 + 自身 conv3_0
            # input = nb[3] + nb[4] + nb[2]
    ...
    def forward(self, input):
        ...
        if self.max_downsample == 16:
            ...
            # Final Fusion (4 inputs, 去掉了 layer 4)
            Final_x0_4 = self.conv0_4_final(
                torch.cat([
                    # self.up_16(self.conv0_4_1x1(x4_0)), # REMOVED
                    self.up_8(self.conv0_3_1x1(x3_1)),  # 16通道
                    self.up_4(self.conv0_2_1x1(x2_2)),  # 16
                    self.up(self.conv0_1_1x1(x1_3)),    # 16
                    x0_4                                # 16
                ], 1))    # cat为64

        # --- Output ---
        # MMDet backbone 通常输出一个元组 tuple
